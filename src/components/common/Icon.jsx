/*
 * Hand-drawn icon set — no icon library. New paths have to match the existing
 * geometry (24x24 box, no fill, 1.75 stroke, round caps) or they read as a
 * foreign glyph next to the others. One weight only; never mix outline and
 * solid on the same surface.
 *
 * Everything inherits currentColor and sizes off the text box, so Tailwind
 * height and width classes work as they would for any inline SVG.
 */
...
const PATHS = {
  ...
  // Kart venue
  // Two checker tiles, not a 2x2 grid — the grid version reads as a cliché.
  flag:
  ...
  // Angular on purpose; the rounded lightning shape reads as toy-like.
  bolt:
  ...
  // Social — redrawn rather than imported, so the family stays consistent
  facebook:
